    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        tableView.setRowFactory( tv -> {
            ...
            row.setOnMouseClicked(event -> {
                curTime = row.getItem().getFileName();
...
//                    FileTreeItem item = getItemFromAddress(tCurrDir.getText());
//                    ObservableList<FileTreeItem> list = ChildArrayHelper.getChildren(item);
//                    for(FileTreeItem i1:list) {
//                        System.out.println(i1.getFile());
//                    }
        ...
        bFind.setOnMouseClicked(event -> {
                FileTreeItem item = getItemFromAddress(tCurrDir.getText());
                ObservableList<FileTreeItem> list = ChildArrayHelper.getChildren(item);
                if(!tFind.getText().equals("")){
                    for(FileTreeItem i1:list){
                        if(!i1.getFile().getName().contains(tFind.getText()))
                            i1.getParent().getChildren().remove((TreeItem)i1);
                    }
                    showItems(item);
                }
                else{
        ...
        bRename.setOnMouseClicked(event -> {
                FileTreeItem item = getItemFromAddress(tCurrDir.getText());
                item.getChildren().remove(0);
                ObservableList<FileTreeItem> list = ChildArrayHelper.getChildren(item);
                for(FileTreeItem i1:list){
                    System.out.println(i1.getFile().getName());
                }
            }
        );
    }
